...
- **매매 가정**: 신호가 발생한 다음 거래일 시초가로 전액 전환하며, 슬리피지를 적용해 보수적으로 체결가를 추정합니다(`config.py`의 `BACKTEST_SLIPPAGE`: 매수 +%, 매도 -%). 수량은 TQQQ 정수 주 단위로만 매매합니다.
...
- **방어 자산**: 현재 방어는 현금(CASH)만 사용합니다. 추후 다른 방어 자산을 넣을 경우에도 동일한 로직으로 전환/평가합니다.
